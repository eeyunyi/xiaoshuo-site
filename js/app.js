...
function handlePopState() {
    ...
    if (!state || state.view === 'home') {
      ...
      // 核心修复：把 state.subCatId 传给分类渲染函数
      renderCategory(state.catId, false, state.subCatId);
 ...
 /* ========== 分类页：子分类 + 角色列表 ========== */
  function renderCategory(catId, doPush = true, subId = null) {
    const cat = NOVEL_DATA.categories.find(c => c.id === catId);
    if (!cat) return;

    // 核心修复：如果有传入指定的子分类，就用指定的；否则才用第一个
    let targetSub = null;
    if (subId) {
      targetSub = cat.subCategories.find(s => s.id === subId);
    }
    if (!targetSub) {
      targetSub = cat.subCategories[0];
    }
    
    currentSubCategoryId = targetSub?.id;

    if (doPush) pushState('category', catId, currentSubCategoryId);
    ...
    app.innerHTML = `
      <div class="category-view animate-fadeIn" style="--cat-color: ${cat.color}">
        ...
        <div class="characters-area" id="charactersArea">
          ${renderCharacterGrid(targetSub)}
        </div>
      </div>
    `;

    queueMasonryLayout();
    window.scrollTo(0, 0); // 确保返回时页面置顶
  }
...
function switchSub(catId, subId) {
    ...
    // 核心修复：更新当前的历史记录状态，记住选中的子分类 (用 replaceState 不会增加多余的后退层级)
    history.replaceState({ view: 'category', catId: catId, subCatId: subId }, '', '#');
